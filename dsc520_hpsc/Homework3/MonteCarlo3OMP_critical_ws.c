...
// Does a Weak Scaling and uses the Critical Parallelization approach

int main(int argc, char **argv){

	const int d = pow(2,6);
	
	//double thread_count[64] = {0}; //Initialize all threads to 0
	...
	int threads = 0.0;         //Initialize number of threads to be allocated
	for(int t=0; t<6; t++){
		threads += pow(2,t);
	}
	//int thread_count[threads] = {0};
	double lower, upper;
	lower = -0.5;
	upper = 0.5;
	//unsigned long long N = threads * atoll(argv[1]);	//Cognizant with the keyword argument argv to intake a user input //DON'T USE <conio.h> package !!!
	unsigned long long N = threads * pow(10,5);
...
double func(double *x, int dim){
	...
	double value = 0.0;
	#pragma omp parallel for //private(value) //IF we make the "value" private here, it'll not be usable in our function calls & the integral = 0
	for(int i=1; i<dim; i++){
...
double monteCarlo(double lower, double upper, int N){
		...
		for(int i = 0; i<N; i++){ //Outer for-loop repeats the integration for N trials laid down in the shell script 
			...
			for(int j = 0; j < dim; j++){ //Inner for loop basically creates the "dim" dimensional box and integrates the function "dim" times
				...
				x[j] = random_number() - 1;
				//x[j] = 1; //Test case to check if the integrand is correct
