...
double func(double x,double y);                              //Function to store the integrand
...
int main(int argc, char **argv){
	...
	double estimate = monteCarlo(lower,upper,N); //To calculate the integral value
	//printf("%i   %.4f\n",N,estimate);
...
double monteCarlo(double lower, double upper, int N){
		...
		double sum = 0.0;  //This will store the summation value
		
			for(int i = 0; i<N; i++){
				for(int j = 0; j < 2; j++){
				//Generate a random number within the limits of the integration
				double rand_num = random_number() - 1;
			       	double rand_num2 = random_number() - 1;			
				double func_value = func(rand_num,rand_num2);  //Pass the requisite random number into the integrand i.e. the parent function
